...
router.get('/:userId/:targetUserId', auth, async (req, res) => {
  try {
    ...
    // Verify the authenticated user is accessing their own messages
    if (req.user.userId != userId) {
      console.log(`Auth mismatch: ${req.user.userId} trying to access ${userId}'s messages`);
...
router.get('/requests/:userId', auth, async (req, res) => {
  try {
    ...
    // Verify the authenticated user is accessing their own message requests
    if (req.user.userId != userId) {
